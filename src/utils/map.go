...
func (s *IntInt64Map) Init() {
	s.Lock()
	defer s.Unlock()
	s.m = make(map[int]int64)
}

func (s *IntInt64Map) Get(key int) (int64, bool) {
	s.Lock()
	defer s.Unlock()
	_, exist := s.m[key]
	if exist {
...
func (s *IntInt64Map) Delete(key int) {
	s.Lock()
	defer s.Unlock()
	delete(s.m, key)
}

func (s *IntInt64Map) GetAll() map[int]int64 {
	return s.m
}

func (s *IntInt64Map) Insert(key int, value int64) {
	s.Lock()
	defer s.Unlock()
	s.m[key] = value
}

func (s *IntInt64Map) Increment(key int) int64 {
	s.Lock()
	defer s.Unlock()
	v, exist := s.m[key]
	if !exist {
...
func (s *IntInt64Map) Set(key int, v int64) {
	s.Lock()
	defer s.Unlock()
	s.m[key] = v
}

func (s *IntInt64Map) IsExist(key int, v int64) bool {
	s.Lock()
	defer s.Unlock()
	tmp, exi := s.m[key]
	if !exi {
